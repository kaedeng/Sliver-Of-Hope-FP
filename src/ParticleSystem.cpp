...
ParticleSystem::~ParticleSystem() {
    // Note: We don't delete static buffers here as they may be used by other instances
}
...
void ParticleSystem::spawnBurst(const glm::vec3& position, int numParticles) {
    for (int i = 0; i < numParticles; ++i) {
        ...
        float elevation = (static_cast<float>(rand()) / RAND_MAX - 0.5f) * M_PI * 0.5f;
        ...
        particle.maxLifetime = 1.0f + (static_cast<float>(rand()) / RAND_MAX) * 1.0f;
...
void ParticleSystem::draw(GLuint shaderProgramHandle,
                          GLint mvpMatrixLoc,
                          GLint textureLoc,
    ...
    for (const auto& particle : _particles) {
        ...
        // Add rotation
        modelMtx = glm::rotate(modelMtx, particle.rotation, glm::vec3(0.0f, 0.0f, 1.0f));
